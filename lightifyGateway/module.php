...
class lightifyGateway extends IPSModule
{
  ...
  public function Create() {
    ...
    //Store at runtime
    $this->SetBuffer("method", vtNoValue);
    ...
    $this->RegisterAttributeBoolean("sendQueue", false);
  ...
  public function ApplyChanges() {
    ...
    //Atrributes
    $this->WriteAttributeBoolean("sendQueue", false);
/*
    $this->WriteAttributeString("deviceBuffer", vtNoString);
    $this->WriteAttributeString("cloudDevices", vtNoString);
    $this->WriteAttributeString("groupBuffer", vtNoString);
    $this->WriteAttributeString("cloudGroups", vtNoString);
    $this->WriteAttributeString("groupDevices", vtNoString);
    $this->WriteAttributeString("sceneBuffer", vtNoString);
    $this->WriteAttributeString("cloudScenes", vtNoString);
*/
    //Buffer
  ...
  public function ReceiveData($JSONString) {
    ...
    if (strlen($data) >= classConstant::BUFFER_REPLY_LENGTH) {
      $this->WriteAttributeBoolean("sendQueue", true);
  ...
  private function getRefreshToken() : string {
    ...
    if (!empty($data) && time() < $data->expires_in) {
      if ($this->debug % 2) {
        ...
        //$this->SendDebug("<Gateway|getRefreshToken:expires>", date("Y-m-d H:i:s", (int)time()+$data->expires_in), 0);
...
*/
  ...
  private function waitReceive() : void {
    $this->WriteAttributeBoolean("sendQueue", false);

    for ($x = 0; $x < 300; $x++) {
      if ($this->ReadAttributeBoolean("sendQueue")) {
        $this->WriteAttributeBoolean("sendQueue", false);
        break;
      }
