...
class lightifyConnect extends stdClass {
  ...
  public function sendRaw($command, $flag, $args = null) {
    ...
    if ($this->debug % 4 || $this->message) {
      ...
      if ($this->debug % 4) {
	      IPS_SendDebug($this->parentID, "<LIGHTIFY|SOCKET|SENDRAW:WRITE>", $info, 0);
	    }

      if ($this->message) {
	      IPS_LogMessage("SymconOSR", "<LIGHTIFY|SOCKET|SENDRAW:WRITE>   ".$info);
	    }
    }
    ...
    if (false !== ($bytes = @fwrite($this->lightifySocket, $data, $length))) {
      if ($bytes == $length) {
        ...
        if ($this->debug % 3 || $this->message) {
          ...
          if ($this->debug % 3) {
	          IPS_SendDebug($this->parentID, "<LIGHTIFY|SOCKET|SENDRAW:READ>", $info, 0);
	        }

          if ($this->message) {
	          IPS_LogMessage("SymconOSR", "<LIGHTIFY|SOCKET|SENDRAW:READ>   ".$info);
	        }
        }
    ...
    IPS_SendDebug($this->parentID, "<LIGHTIFY|SOCKET|SENDRAW:ERROR>", $error, 0);
    IPS_LogMessage("SymconOSR", "<LIGHTIFY|SOCKET|SENDRAW:ERROR>   ".$error);
  }
